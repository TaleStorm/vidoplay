...
const Comments = (data: CommentsProps) => (
  	<div className="ml-7">
		  ...
		  {data.comments.map((comment, i) => {    
            return (
				<div className="" key={i}>
					<a href={`${comment.url}`} className="text-base hover:text-gray-900 ml-7 block mt-5">
					...
					<h4 className="text-xs font-roboto text-mainText font-normal block mt-2 opacity-80">
						{comment.comment}
					</h4>
					<h4 className="text-xs font-roboto text-mainText font-normal block mt-4 opacity-80">
						{comment.film}
					</h4>
					</a>
				</div>
