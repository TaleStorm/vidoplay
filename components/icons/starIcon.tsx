const StarIcon = ({index, hoveredRating, setHoveredRating, rating, setRating}) => {
    return (
    <svg 
    onMouseOver={() => {
    ...
    className={`cursor-pointer transition-all duration-200`} width="30" height="30" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        fill={`${!rating ? hoveredRating >= index ? "#EFCF33" : "transparent" : rating >= index ? "#EFCF33" : "transparent"}`}   
        stroke={`${!rating ? hoveredRating >= index ? `transparent` : "#8A898F" : rating >= index ? "transparent" : "#8A898F"}`} 
        strokeWidth="2" />
    </svg>
    )
}
