    ...
    <div className="">
              <Menu as="div" className="relative inline-block text-left w-full">
                {({ open }) => (
                  <>
                    <div>
                      <Menu.Button className={`inline-flex justify-center w-full rounded-md shadow-sm px-3 py-2 bg-playerMain text-sm font-roboto`} >
                        <div className={`inline-flex w-full px-3 justify-between items-center ${open && "text-orange"}`}>
                          <div className={`text-h2-mobile whitespace-nowrap`}>{state}</div>
                      ...
                      >
                        ...
                        {datas.map((data, i) => {
                            return (
                              <Menu.Item key={i}>
                                {({ active }) => (
                                  <button onClick={() => {setState(data)}} className={`${active && 'bg-black text-white'} w-full py-2 hover:bg-orange text-left px-4 ${i===0 && "rounded-t-lg"} ${i===datas.length-1 && "rounded-b-lg"} text-h2-mobile`}>
                                    {data}
                                  </button>
                                )}
                              </Menu.Item>
