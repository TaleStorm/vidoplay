...
const FilmCategorySliderCard = (data: FilmCategorySliderCardProps) => (
  	<div key={data.key} className="bg-cardBackground w-full relative">
		<div>
			<div className={"bg-cover relative bg-center bg-gray-300 h-"+data.imageSize +" bg-"+data.image}>
		...
		<div className="my-6">
			...
			<p className="text-sm font-roboto text-mainText mx-4 opacity-70 mb-6">
				{data.description}
			</p>
		</div>
		<div className="flex flex-row py-3 justify-start flex-wrap mx-4">
			{data.tags.map((tag, i) => {    
